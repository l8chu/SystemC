...
void zigzag_quant(double data[8][8], FILE *output) {
	signed char to_write[8][8]; // this is the rounded values, to be written to the file
	char last_non_zero_value = 0; // this is the index to the last non-zero value in a block

	// zigzag the data array and copy it back to to_write
	// round the values to the nearest integer and find out the index to the last
	// non-zero value in the 8x8 block
	for (unsigned char i = 0; i < 64; i++) {
		to_write[zigzag_tbl[i]/8][zigzag_tbl[i]%8] = rnd(data[i/8][i%8] / quant[i/8][i%8]);
			if (to_write[i/8][i%8] != 0)
